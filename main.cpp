...
int main() {
    ...
    for (DIM1 i = 0; i < n; i++) {
        ...
        for (DIM1 j = 0; j < n; j++) {
            cin >> temp;
            if (temp != -1) matriz[i][j] = temp;
            else matriz[i][j] = MAX_DIM2;
        }
    }
    ...
    for (DIM1 i = 0; i < n; i++) {
        ...
        for (DIM1 j = 0; j < n; j++) {
            if (matriz[i][j] != MAX_DIM2) matriz[i][j] -= min;
        }
        reduced_counter += min;
        }
    }
    ...
    cout << endl << "RC: " << reduced_counter << endl;
    ...
    for (DIM1 i = 0; i < n; i++) {
        ...
        //GET MIN VALUE
        for (DIM1 j = 0; j < n; j++) {
            if (i != h_r && j != h_c && min > matriz[i][j]) min = matriz[i][j];
        }
        //SUBSTRACT MIN VALUE
        if (min != 0 && min != MAX_DIM2) {
        for (DIM1 j = 0; j < n; j++) {
    ...
    cout << endl << "NRC: " << new_reduced_counter << endl;
